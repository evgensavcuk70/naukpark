...
⚠️ У файлі `.env`, який раніше лежав у цьому архіві, та у `docker-compose.yml` був захардкоджений реальний пароль від бази даних. Перед публікацією репозиторію **обов'язково змініть цей пароль** (і в БД, і в новому `.env`), оскільки він більше не вважається таємним. `docker-compose.yml` вже виправлено — пароль тепер підтягується зі змінних середовища (`.env`), а не прописаний у файлі напряму.
